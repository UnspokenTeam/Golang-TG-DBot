...
func FAction(ctx context.Context, upd telego.Update, services *service_wrapper.Services) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		lastTime, _ := tx.GetLastTimeFAction(txCtx, querier.GetLastTimeFActionParams{
			ChatTgID: upd.Message.Chat.ID,
			UserTgID: upd.Message.From.ID,
		})
		sendFail(ctx, cooldown, services.ConfigCache, *lastTime, upd, "f_action_text_fail_pattern")
	}
	if err != nil {
		return
	}
